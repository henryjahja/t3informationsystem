		...
		aux		= {
				...
				return $.Deferred(
					function( dfd ) {
						...
						if( expanded ) {
							...
							//$el.find('.va-content').fadeOut(settings.contentAnimSpeed);
							$el.find('.va-content').animate({ 
								height:483, 
								height:0
								}, settings.contentAnimSpeed);
							
							// THIS FUNCTION CALL BEFORE SLICE CLOSE
							settings.beforeSliceClose();
						...
						// if it's closed..
							...
							// ADD ACTIVE CLASS ON CURRENT SLICE
							$el.addClass('active-slice');
							$el.removeClass('other-slice');
							
							// THIS FUNCTION CALL BEFORE SLICE OPEN
							settings.beforeSliceOpen();
						...
						$el.stop()
						   .animate( animParam, settings.animSpeed, settings.animEasing, function() {
								...
								if($el.data('expanded')==false){
									// THIS FUNCTION CALL AFTER SLICE CLOSE
									settings.afterSliceClose();
									
									// REMOVE ACTIVE CLASS AFTER SLICE CLOSE
									$el.removeClass('active-slice');
									$el.addClass('other-slice');
									
								}else{
									// THIS FUNCTION CALL AFTER SLICE OPEN
									settings.afterSliceOpen();
								}
						   }).find('.va-title')
						   .stop()
						   .animate({
						...
						$others.each(function(i){
							...
							$other.stop()
								  ...
								  .end()
								  .find('.va-content').animate({ 
										height:483, 
										height:0
										}, settings.contentAnimSpeed);
								  //.find('.va-content').fadeOut(settings.contentAnimSpeed);
			...
			slide			: function( dir, $slices, $navNext, $navPrev, settings ) {
				...
				if( ( dir === 1 && !aux.canSlideDown( $slices, settings ) ) || 
					...
					//SET ACTIVE SLICE CLASS
					$nextSlice.addClass('active-slice');
					$nextSlice.removeClass('other-slice');
					...
					setTimeout(function() {
						...
						// THIS FUNCTION REMOVE ACTIVE CLASS AFTER SLICE CALL
						$currentSlice.removeClass('active-slice');
						$currentSlice.addClass('other-slice');
				...
				$slices.each(function(i) {
					...
					if( !settings.savePositions || cache.idx_expanded === -1 )
						...
						if( i === $nextSlice.index() ) {
							...
							//SET ACTIVE SLICE CLASS
							$slice.addClass('active-slice');
							$slice.removeClass('other-slice');
							...
							$slice.stop()
						          .animate({
									   ...
									   $slice.find('.va-content').animate({ 
										height:0, 
										height:483
										}, settings.contentAnimSpeed);
									   //.find('.va-content').fadeIn( settings.contentAnimSpeed );
						...
						// if the slice is the one opened, lets close it
							...
							$slice.stop()
						          .animate({ 
									  ...
									  if($slice.data('expanded') == false){
										  	// THIS FUNCTION CALL AFTER SLICE CLOSE
											settings.afterSliceClose();
											
											// REMOVE ACTIVE CLASS AFTER SLICE CLOSE
											$slice.removeClass('active-slice');
											$slice.addClass('other-slice');
										  }
								})
								  .find('.va-title')
								  .stop()
			...
			init 		: function( options ) {
				...
				if( this.length ) {
					...
					var settings = {
						...
						animStart		: function(){},
						animEnd			: function(){},
						beforeSliceOpen		: function(){},
						afterSliceOpen		: function(){},
						beforeSliceClose	: function(){},
						afterSliceClose		: function(){}
					};
					...
					return this.each(function() {
						...
						// click event
						$slicesCtrl.bind('click.vaccordion', function(e) {
